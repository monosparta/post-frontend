        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <TransitionChild as="template" enter="ease-out duration-300"
            ...
            <DialogPanel
              ...
              <div>
                <div
                  :class="[props.type === 'add' || props.type ==='update' ? 'bg-indigo-100' : 'bg-red-100' , 'mx-auto flex h-12 w-12 items-center justify-center rounded-full']">
                  <PencilAltIcon v-if="props.type === 'add' || props.type === 'update'" class="h-6 w-6 text-indigo-600"
                    aria-hidden="true" />
              ...
              <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                <button type="button"
                  ...
                  @click="emit('click');emit('confirm')">確定</button>
                <button type="button"
                  class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:col-start-1 sm:mt-0 sm:text-sm"
                  @click="emit('click')" ref="cancelButtonRef">取消</button>
              </div>
            </DialogPanel>
          </TransitionChild>
        </div>
